...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.div
      ...
      whileHover={!product.soldOut ? { y: -8 } : {}}
      transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
    >
      {/* Línea superior verde */}
      <div className={`h-[2px] transition-all duration-500 ${hovered && !product.soldOut ? 'bg-[#a3af74]' : 'bg-[#a3af74]/30'}`} />

      {/* Imagen */}
      <div className="relative overflow-hidden bg-white" style={{ height: '320px' }}>
        <motion.div
          className="absolute inset-0 bg-contain bg-center bg-no-repeat"
          style={{ backgroundImage: `url('${placeholderImages[product.id]}')`, padding: '16px' }}
          animate={{ scale: hovered && !product.soldOut ? 1.05 : 1 }}
        ...
        <div className="absolute top-4 right-4">
          <span className="bg-white/90 text-[#a3af74] text-[10px] px-3 py-1 tracking-[0.2em] uppercase border border-[#a3af74]/30">
            {product.format}
          </span>
        </div>
      </div>
      ...
      {/* Separador */}
      <div className="mx-6 h-px bg-[#1C1C1A]/6" />
        ...
        <div className="flex items-center justify-between mt-8 pt-6 border-t border-[#1C1C1A]/8">
          ...
          {!product.soldOut && (
            <Link
              ...
              className="flex items-center gap-2.5 px-6 py-3 bg-[#a3af74] text-white text-[10px] tracking-[0.25em] uppercase hover:bg-[#1C1C1A] transition-colors duration-400"
            >
